...
class SupportsAudioOutput(Protocol):
    ...
    def decode_audio_tokens(
        ...
        Args:
            token_ids: Audio token IDs generated by the LLM backbone.
            model_instance: The loaded model instance whose vocoder will be
                used for decoding.  Must have a ``tts`` attribute that was
                initialised with ``model.init_tts()``.

        Returns:
            Float32 waveform array with shape ``[num_samples]``.

        Raises:
            RuntimeError: If ``model_instance`` has no initialised ``tts``
                attribute (e.g. quantised model or TTS not initialised).
        """
        ...
