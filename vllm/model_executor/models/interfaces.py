...
class SupportsAudioOutput(Protocol):
    ...
    def decode_audio_tokens(
        ...
        Called on the serving model instance.  The model must have ``tts``
        and ``vocos`` attributes initialised (i.e. started with
        ``enable_audio_output=True`` and ``init_tts()`` called).
        ...
        Raises:
            RuntimeError: If TTS is not initialised on this model instance
                (e.g. ``enable_audio_output`` not set or ``init_tts()`` not
                called).
        """
        ...
